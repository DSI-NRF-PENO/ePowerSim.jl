...
# # (dx, x, (src_u, dst_u, f_t, y_t), para, t)

# function Line_cim!(de, e, (v_s, v_d, f_t, y_t), p, t) # (de, e, p_agg, t)
...
#     isd      = Y * [us, ud]
#     is       = isd[1]   # computed current
#     id       = isd[2]   # computed current
...
# @kwdef struct Line_cim <: PiModelEdge
#     from::String   = "bus0"
#     to::String     = "bus0"
#     comp_type::Symbol = :Line_cim
#     name::String   = from * "-" * to
#     orientation::Tuple{Int64, Int64} = ( parse(Int, split(lowercase(from), "bus")[2] ), parse(Int, split(lowercase(to), "bus")[2]) ) 
#     y::ComplexF64       = 1.0 + im * 0.0
#     y_shunt_km::Float64 = 0.0
#     y_shunt_mk::Float64 = 0.0
#     t_ratio::ComplexF64                         = 1.0 + im * 0.0
#     Ybr::Matrix{ComplexF64}                     = PiModel(y, y_shunt_km, y_shunt_mk, 1, 1)
#     xs::Float64                                 = -1*imag(y)/(abs(y))^2
#     bi::Float64                                 = 1/(xs * abs(t_ratio))
#     YbrDC::Matrix{ComplexF64}                   = [1/(abs(t_ratio))^2  -1/conj(t_ratio); -1/t_ratio 1] * 1/(im * xs)
#     BbrDC::Matrix{Float64}                      = bi * [1 -1; -1 1]
#     Pshift::Vector{Float64}                     = angle(t_ratio) * bi * [-1, 1]
   
#     algebraic_vars_syms::Vector{Symbol}         = Symbol[]
#     state_vars_syms::Vector{Symbol}             = Symbol[:is_r, :is_i, :id_r, :id_i]
#     syms::Vector{Symbol}                        = Symbol[state_vars_syms...;algebraic_vars_syms...]
#     dim::Int64                                  = length(syms) 
#     mass_matrix::Diagonal{Int64, Vector{Int64}} = DAE_MassMatrix(length(state_vars_syms), length(algebraic_vars_syms))
#     param::Vector{Symbol}                       = Symbol[:y, :y_shunt_km, :y_shunt_mk]
#     func::Vector{Function}                      = Function[Line_cim!]
#     control_sig::Vector{Float64}     = Float64[1.0]
#     control_sig_syms::Vector{Symbol} = Symbol[:c1]
#     output_sig::Vector{Float64}      = Float64[1.0]
#     output_sig_syms::Vector{Symbol}  = Symbol[:o1] 
    
# end


# function Line_pim!(de, e, (v_s, v_d, f_t, y_t), p, t) # (de, e, p_agg, t)
...
# @kwdef struct Line_pim <: PiModelEdge
    
#     from::String
#     to::String

#     comp_type::Symbol = :Line_pim
...
#     algebraic_vars_syms::Vector{Symbol}         = Symbol[]
#     state_vars_syms::Vector{Symbol}             = Symbol[:ph, :qh, :pk, :qk]
#     syms::Vector{Symbol}                        = Symbol[state_vars_syms...;algebraic_vars_syms...]
#     dim::Int64                                  = length(syms) 
#     mass_matrix::Diagonal{Int64, Vector{Int64}} = DAE_MassMatrix(length(state_vars), length(algebraic_vars))
#     param::Vector{Symbol}                       = Symbol[:y, :y_shunt_km, :y_shunt_mk]
#     func::Vector{Function}                      = Function[Line_pim!]
#     control_sig::Vector{Float64}     = Float64[1.0, 1.0, 1.0, 1.0]
#     control_sig_syms::Vector{Symbol} = Symbol[:c1, :c2, :c3, :c4]
#     output_sig::Vector{Float64}      = Float64[1.0, 1.0, 1.0, 1.0]
#     output_sig_syms::Vector{Symbol}  = Symbol[:o1, :o2, :o3, :o4]     
# end


# function AC_Transformer!(de, e, (v_s, v_d, f_t, y_t), p, t) # (de, e, p_agg, t)
...
# @kwdef struct AC_Transformer <: PiModelEdge
#     from::String
#     to::String

# comp_type::Symbol = :AC_Transformer
...
#     y_shunt_km::Float64  = 0.0
#     y_shunt_mk::Float64  = 0.0
#     t_ratio::ComplexF64  = 1.0 + im * 0.0   # = Complex(1.0, 0.0)
#     # Ybr::Matrix{ComplexF64} = [(y + 1im*y_shunt_km)*1/(abs(t_ratio))^2  -y*1/conj(t_ratio); -y*1/t_ratio y + 1im*y_shunt_mk]
#     Ybr::Matrix{ComplexF64} = PiModel(y, y_shunt_km, y_shunt_mk, t_ratio, 1)
#     xs::Float64 = -1*imag(y)/(abs(y))^2
#     bi::Float64 = 1/(xs * abs(t_ratio))
#     YbrDC::Matrix{ComplexF64} = [1/(abs(t_ratio))^2  -1/conj(t_ratio); -1/t_ratio 1] * 1/(im * xs)
#     BbrDC::Matrix{Float64} = bi * [1 -1; -1 1]
...
#     syms::Vector{Symbol} = Symbol[state_vars_syms...;algebraic_vars_syms...]
#     dim::Int64 = length(syms)
#     # mass_matrix::UniformScaling{Bool} = I
#     # mass_matrix::Diagonal{Int64, Vector{Int64}} = MassMatrix()    
#     mass_matrix::Diagonal{Int64, Vector{Int64}} = DAE_MassMatrix(length(state_vars_syms), length(algebraic_vars_syms))
#     param::Vector{Symbol} = Symbol[:y, :y_shunt_km, :y_shunt_mk, :t_ratio]
#     func::Vector{Function} = Function[AC_Transformer!]
#     control_sig::Vector{Float64}     = Float64[1.0, 1.0, 1.0, 1.0]
#     control_sig_syms::Vector{Symbol} = Symbol[:c1, :c2, :c3, :c4]
#     output_sig::Vector{Float64}      = Float64[1.0, 1.0, 1.0, 1.0]
#     output_sig_syms::Vector{Symbol}  = Symbol[:o1, :o2, :o3, :o4]     
...
# function AC_Transformer_pim!(de, e, (v_s, v_d, f_t, y_t), p, t) # (de, e, p_agg, t)
...
# @kwdef struct AC_Transformer_pim <: PiModelEdge
#     from::String
#     to::String

#     comp_type::Symbol = :AC_Transformer_pim
...
#     mass_matrix::Diagonal{Int64, Vector{Int64}} = DAE_MassMatrix(length(state_vars_syms), length(algebraic_vars_syms))
#     param::Vector{Symbol} = Symbol[:y, :y_shunt_km, :y_shunt_mk, :t_ratio]
#     func::Vector{Function} = Function[AC_Transformer_pim!]
#     control_sig::Vector{Float64}     = Float64[1.0, 1.0, 1.0, 1.0]
#     control_sig_syms::Vector{Symbol} = Symbol[:c1, :c2, :c3, :c4]
#     output_sig::Vector{Float64}      = Float64[1.0, 1.0, 1.0, 1.0]
#     output_sig_syms::Vector{Symbol}  = Symbol[:o1, :o2, :o3, :o4]     
